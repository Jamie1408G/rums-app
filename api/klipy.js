...
function simplify(item) {
  ...
  const preview = formats.nanogif || formats.tinygif || formats.mediumgif || formats.gif;
  const mp4 = formats.mp4 || formats.loopedmp4 || formats.tinymp4 || null;
  const tinymp4 = formats.tinymp4 || formats.mp4 || formats.loopedmp4 || null;
  if (!full?.url) return null;
  return {
    id: safeText(item.id, 80),
    title: safeText(item.content_description || item.title || 'GIF', 180),
    url: full.url,
...
export default async function handler(req, res) {
  const key = getKey();
  if (!key) {
    return res.status(503).json({ error: 'GIF service is not configured yet. Add APP_KEY in Vercel Environment Variables.' });
  }
  ...
  try {
    ...
    const limit = Math.max(8, Math.min(30, Number(req.query?.limit) || 18));
    ...
    url.searchParams.set('media_filter', 'gif,tinygif,nanogif,mp4,tinymp4');

    const response = await fetch(url, { headers: { Accept: 'application/json' } });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) return res.status(response.status).json({ error: payload?.error || 'GIF request failed.' });
    ...
    return res.status(500).json({ error: 'Could not load GIFs.' });
  }
}
